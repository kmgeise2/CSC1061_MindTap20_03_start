...
{
    // First build the graph, then add weights to the weight array
    // Read from the input file to build the graph
    ifstream infile;
    char fileName[50];
    ...
    cout << "Enter input file name (Ch20_Ex3Data.txt or dataSet2.txt): ";
    cin >> fileName;
    cout << endl;
    ...
    // Set each (row, column) to infinity
    ...
    while (infile)
    {
        ...
        {
            infile >> weight;
            //Update the weights matrix (row, column)
            // with the weight 
